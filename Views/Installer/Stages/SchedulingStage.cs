...
public static class SchedulingStage
{
    ...
    public static async Task Run()
    {
        ...
        var actions = new List<(Func<string> TitleFunc, Func<Task> Action, Func<bool> Condition)>
        {
        ...
        {
            var currentTitle = filteredActions[i].TitleFunc();
            var previousTitleCheck = i > 0 ? filteredActions[i - 1].TitleFunc() : string.Empty;

            if (i == 0 || currentTitle != previousTitleCheck)
            {
                groupedTitleCount++;
            }
        }
        ...
        for (int i = 0; i < filteredActions.Count; i++)
        {
            var (titleFunc, action, _) = filteredActions[i];
            var title = titleFunc();
