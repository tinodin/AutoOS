...
public static class SecurityStage
{
	public static async Task<List<(string Title, Func<Task> Action, Func<bool>? Condition)>> GetActions()
	{
		...
		[
			...
			("Disabling Windows Defender", async () => RegistryHelper.RunAs(RegistryHelper.Identity.TrustedInstaller, () => ServicesHelper.StopService("wscsvc")), () => WindowsDefender == false),
			("Disabling Windows Defender", async () => RegistryHelper.RunAs(RegistryHelper.Identity.TrustedInstaller, () => { string file = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Windows Defender", "MsMpEng.exe"); if (File.Exists(file)) File.Move(file, file + "e"); }), () => WindowsDefender == false),
