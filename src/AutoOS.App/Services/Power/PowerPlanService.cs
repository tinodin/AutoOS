...
public sealed class PowerPlanService : IPowerPlanService
{
	...
	public IReadOnlyList<Plan> GetPowerPlans()
	{
		var plans = new List<Plan>();
		foreach (Guid scheme in PowerHelper.EnumerateSchemes())
			plans.Add(new Plan(scheme, PowerHelper.ReadFriendlyName(scheme, null, null), PowerHelper.ReadDescription(scheme)));
		return plans;
	}

	public Guid GetActivePowerPlan() => PowerHelper.ReadActiveScheme();

	public (Plan Plan, IReadOnlyList<Subgroup> Subgroups, IReadOnlyDictionary<Setting, Value> Values) ReadPowerPlan(Guid scheme)
	{
	...
	public Value? ReadValue(Guid scheme, Guid subgroupGuid, Guid settingGuid)
	{
	...
	public void SetActivePowerPlan(Guid scheme) => PowerHelper.PowerSetActiveScheme(scheme);

	public Plan UpdatePowerPlanMetadata(Plan plan, string name, string description)
	{
	...
	public Guid DuplicatePowerPlan(Guid scheme, string name, string description) => PowerHelper.DuplicateScheme(scheme, name, description);

	public void ExportPowerPlan(Guid scheme, string path) => PowerHelper.ExportPowerScheme(scheme, path);

	public void DeletePowerPlan(Guid scheme) => PowerHelper.DeleteScheme(scheme);

	public Guid ImportPowerPlan(string filePath) => PowerHelper.ImportPowerScheme(filePath);

	public async Task RestoreDefaultPowerPlansAsync()
	{
	...
	public void SaveChanges(Guid scheme, IEnumerable<(Setting Setting, Value Value)> changes)
	{
